...
class MM:
    ...
    def treat_normal_proof(self, proof: list[str]) -> list[Stmt]:
        ...
        for label in proof:
            # Handle unknown proof steps per spec 4.4.6 (following metamath.exe mmveri.c:47)
            if label == '?':
                stack.append([])  # Push placeholder like hypothesis
                continue
...
if __name__ == '__main__':
    ...
    transformed_metta_file = args.transformed_metta_file
    if transformed_metta_file:
    ...
    if transformed_metta_file and transform_log:
        with open(transformed_metta_file, 'w') as out:
