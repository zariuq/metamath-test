...
[FIXED 2025-10-07] The above limitation has been removed. mmverify_pure.py
now correctly handles interleaved $f and $e hypotheses in RPN order per spec.
...
class Frame:
    ...
    def __init__(self) -> None:
        ...
        # are needed.
        # RPN order tracking: records interleaved $f and $e in appearance order
        self.order: list[tuple[str, typing.Union[Fhyp, Ehyp], Label]] = []
...
class FrameStack(list[Frame]):
    ...
    def add_e(self, stmt: Stmt, label: Label) -> None:
        ...
        # Record in RPN order
        frame.order.append(("e", stmt, label))
...
class MM:
    ...
    def add_f(self, typecode: Const, var: Var, label: Label) -> None:
        ...
        # Record in RPN order
        frame.order.append(("f", fhyp, label))
    ...
    def treat_step(self,
        ...
        Now handles RPN-interleaved $f and $e hypotheses correctly.
        """
        ...
        if is_hypothesis(step):
            ...
            # Build RPN-ordered mandatory hypotheses from frames
            # This gives true interleaved order as they appear in source
            mand_vars = {var for _tc, var in f_hyps0}
            rpn_hyps: list[tuple[str, typing.Union[Fhyp, Ehyp]]] = []
            seen_vars: set[Var] = set()

            for fr in self.fs:
                for hyp_type, hyp_data, _label in fr.order:
                    if hyp_type == "f":
                        _typecode, var = hyp_data  # type: ignore
                        if var in mand_vars and var not in seen_vars:
                            rpn_hyps.append(("f", hyp_data))
                            seen_vars.add(var)
                    elif hyp_type == "e":
                        # Include $e if it's in e_hyps0 (active for this assertion)
                        if hyp_data in e_hyps0:  # type: ignore
                            rpn_hyps.append(("e", hyp_data))
            ...
            # Process hypotheses in RPN order (single pass, interleaved)
            subst: dict[Var, Stmt] = {}
            for hyp_type, hyp_data in rpn_hyps:
                entry = stack[sp]
                if hyp_type == "f":
                    typecode, var = hyp_data  # type: ignore
                    if entry[0] != typecode:
                        raise MMError(
            ...
            # Check DV constraints (unchanged)
            for x, y in dvs0:
    ...
    def treat_normal_proof(self, proof: list[str]) -> list[Stmt]:
        ...
        """
        stack: list[Stmt] = []
        self._proof_has_unknown = False  # Track incomplete proofs (spec 4.4.6)
        active_hypotheses = {label for frame in self.fs for labels in (frame.f_labels, frame.e_labels) for label in labels.values()}
        for label in proof:
            # Handle unknown proof steps per spec 4.4.6
            if label == '?':
                self._proof_has_unknown = True
                # Push empty placeholder (like metamath.exe does)
                stack.append([])
                continue
    ...
    def treat_compressed_proof(
        ...
        # Ignore whitespace within compressed payload - Spec §4.4.2
        compressed_proof = ''.join(tok for tok in proof[idx_bloc + 1:] if tok.strip())
        ...
        for ch in compressed_proof:
            if ch == 'Z':
                ...
                # Unknown step in compressed proof - Spec §4.4.6
                proof_ints.append(-2)
            elif 'A' <= ch <= 'T':
        ...
        for proof_int in proof_ints:
            if proof_int == -1:  # save the current step for later reuse
                ...
                n_saved_stmts += 1
            elif proof_int == -2:  # unknown step placeholder
                self._proof_has_unknown = True
                stack.append([])  # Push empty placeholder
            elif proof_int < label_end:
                # proof_int denotes an implicit hypothesis or a label in the
                # label bloc
                label = plabels[proof_int] or ''
                # Handle unknown proof steps per spec 4.4.6
                if label == '?':
                    self._proof_has_unknown = True
                    stack.append([])  # Push empty placeholder
    ...
    def verify(
        ...
        # Handle incomplete proofs with unknown steps (spec 4.4.6)
        if getattr(self, '_proof_has_unknown', False):
            vprint(1, "Warning: Proof is incomplete (contains '?'); conclusion not checked.")
            return  # Skip stack checks for incomplete proofs
